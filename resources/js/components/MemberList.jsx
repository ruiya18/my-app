// resources/js/components/MemberList.jsx
...
const MemberList = () => {
  ...
  const getStatusClasses = (status) => {
    return status.toLowerCase() === 'active'
      ...
      : 'text-gray-600 bg-gray-100 px-2 py-1 rounded-full'; // fallback
  };
              ...
              <tbody>
                {currentMembers.length > 0 ? (
                  currentMembers.map((member) => (
                    <tr key={member.id}>
                      <td className="border-b px-4 py-2">{member.username}</td>
                      <td className="border-b px-4 py-2">{capitalizeWords(member.role)}</td>
                      <td className="border-b px-4 py-2">
                        <span className={getStatusClasses(member.status)}>
                          {capitalizeWords(member.status)}
                        </span>
                      </td>
                      <td className="border-b px-4 py-2">
                        <button
